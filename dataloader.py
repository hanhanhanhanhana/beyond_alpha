...
LastEditTime: 2021-05-13 21:53:47
...
class MarketLoader:
    def __init__(self, constituent_index=None, custom_stocks=None, save_csv=False, save_dir=None):
        '''
        @description: 根据要求输出对应股票池的股票信息
        @param {constituent_index: 各种股指对应的成分股
    ...
    def load_k_history(self, beg=20210101, end=20500101):    
        # beg: 开始日期，20200101
        # end: 截止日期
        ...
        if self.__constituent_index is None and self.__custom_stocks is None:
            dict_code_as_key, dict_date_as_key = mk.k_history(save_csv=self.__save_csv, save_dir=self.__save_dir, beg=beg, end=end)
        # TODO: 实现成分股
        elif self.__constituent_index is not None:
            constituent_index_stocks = []
            dict_code_as_key, dict_date_as_key = mk.k_history(stock_codes=constituent_index_stocks, save_csv=self.__save_csv, save_dir=self.__save_dir, beg=beg, end=end)
        # 用户自定义股票池
        elif self.__custom_stocks is not None:
            dict_code_as_key, dict_date_as_key = mk.k_history(stock_codes=self.__custom_stocks, save_csv=self.__save_csv, save_dir=self.__save_dir, beg=beg, end=end)
